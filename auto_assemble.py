...
def make_text_material(mat_id: str, text: str) -> dict:
    """Create a CapCut text material for a caption entry."""
    return {
        "id": mat_id,
        "type": "text",
        "name": mat_id,
        "content": json.dumps({
            "styles": [{"fill": {"content": {"render_type": "solid", "solid": {"color": [1.0, 1.0, 1.0, 1.0]}}, "enable": True},
                        "range": [0, len(text)]}],
            "text": text
        }, ensure_ascii=False),
        "font_size": 6.5,
        ...
        "font_name": "", "font_id": "",
        "alignment": 1,
        "line_spacing": 0.1, "letter_spacing": 0.0, "line_max_width": 0.82,
        "background_alpha": 0.0, "background_color": "",
        "bold": False, "italic": False,
        "language": "en-US",
        "original_size": [], "words": {"end_time": [], "start_time": [], "text": []}
    }
...
def groq_transcribe(audio_path: str, api_key: str, model: str = 'whisper-large-v3') -> str:
    ...
    # Build multipart body — use verbose_json (Groq doesn't support srt directly)
    body = (
        field('model', model) +
...
class AutoAssembleGUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Auto Video Assembly (CapCut Draft Injector) v6")
        ...
        self.entry_groq = ttk.Entry(lf_files, textvariable=self.var_groq_key, show="*")
        self.entry_groq.grid(row=5, column=1, sticky="ew", padx=8, pady=(4, 2))
        ttk.Button(lf_files, text="💾 Save", width=6,
                   command=self.save_groq_key).grid(row=5, column=2, pady=(4, 2))

        # Re-scan button
        ttk.Button(lf_files, text="↺ Re-scan", command=self.do_scan).grid(
            row=6, column=2, sticky="e", pady=(4, 0))
        ...
        self.var_export_mode = tk.StringVar(value="capcut")

        tk.Radiobutton(lf_output, text="Auto-Inject into CapCut Project",
                       variable=self.var_export_mode, value="capcut",
                       command=self.toggle_export_mode).grid(row=0, column=0, sticky="w", pady=4)
        ...
        self.cb_projects.grid(row=0, column=1, sticky="ew", padx=8, pady=4)
        if project_names:
            self.cb_projects.current(0)
        ttk.Button(lf_output, text="↺", width=3,
                   command=self.refresh_projects).grid(row=0, column=2, padx=(0, 4), pady=4)
        ...
        self.entry_custom_out.grid(row=1, column=1, sticky="ew", padx=8, pady=4)
        self.btn_custom_out = ttk.Button(lf_output, text="Browse",
                                         command=self.browse_output, state="disabled")
        self.btn_custom_out.grid(row=1, column=2, pady=4)
    ...
    def save_groq_key(self):
        key = self.var_groq_key.get().strip()
        if not key:
            messagebox.showwarning("Empty", "Please enter your Groq API key first.")
            return
        cfg = load_config()
        cfg['groq_api_key'] = key
        save_config(cfg)
        self.log("💾 Groq API key saved.")
    ...
    def transcribe_audio(self):
        ...
        if not audio_path or not os.path.exists(audio_path):
            ...
            return
        api_key = self.var_groq_key.get().strip()
        if not api_key:
            messagebox.showerror("Missing Key", "Please enter your Groq API key and click Save.")
            return
        ...
        self.log(f"   Model: whisper-large-v3 | Please wait...")
        self.update_idletasks()

        import threading
        def do_transcribe():
            try:
                srt_content = groq_transcribe(audio_path, api_key)
    ...
    def toggle_export_mode(self):
        ...
        if mode == "capcut":
            self.cb_projects.config(state="readonly")
            self.entry_custom_out.config(state="disabled")
            self.btn_custom_out.config(state="disabled")
        else:
            self.cb_projects.config(state="disabled")
            self.entry_custom_out.config(state="normal")
            self.btn_custom_out.config(state="normal")
    ...
    def run_process(self):
        ...
        if export_mode == "custom":
            ...
            # CapCut auto-saves its in-memory state on shutdown.
            # If we write files while CapCut is running, then kill it,
            # CapCut overwrites our injected content during its exit.
            # Solution: Kill → Wait → Write → Launch
