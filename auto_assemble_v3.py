...
# ── Gemini 2.5 Flash Transcription via vertex-key.com ──────────────────────
...
def gemini_transcribe(audio_path: str, api_key: str,
                      script_text: str = "",
                      model: str = 'gemini-2.5-flash-chat') -> str:
    """Transcribe audio using Gemini 2.5 Flash via vertex-key.com.
    ...
    # 4. POST to vertex-key.com
    url = 'https://vertex-key.com/api/v1/chat/completions'
...
class AutoAssembleGUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Auto Video Assembly v3 — Gemini Transcription & CapCut Creator")
        ...
        # Vertex API key row
        ttk.Label(lf_files, text="Vertex API Key:", foreground="#555").grid(
            row=6, column=0, sticky="w", pady=(4, 2))
        cfg = load_config()
        self.var_vertex_key = tk.StringVar(value=cfg.get('vertex_api_key', ''))
        self.entry_vertex = ttk.Entry(lf_files, textvariable=self.var_vertex_key, show="*")
        self.entry_vertex.grid(row=6, column=1, sticky="ew", padx=8, pady=(4, 2))
        ttk.Button(lf_files, text="💾 Save", width=6,
                   command=self.save_vertex_key).grid(row=6, column=2, pady=(4, 2))
    ...
    def save_vertex_key(self):
        key = self.var_vertex_key.get().strip()
        if not key:
            messagebox.showwarning("Empty", "Please enter your Vertex API key first.")
            return
        cfg = load_config()
        cfg['vertex_api_key'] = key
        save_config(cfg)
        self.log("💾 Vertex API key saved.")
    ...
    def transcribe_audio(self):
        ...
        if not audio_path or not os.path.exists(audio_path):
            ...
            return
        api_key = self.var_vertex_key.get().strip()
        if not api_key:
            messagebox.showerror("Missing Key", "Please enter your Vertex API key and click Save.")
            return
        ...
        self.log(f"   Model: gemini-2.5-flash-chat via vertex-key.com")
        if script_text:
